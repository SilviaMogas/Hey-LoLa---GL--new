...
/**
 * Default shelters shown on /foundation when the Firestore `shelters`
 * collection is empty. Real NYC partners + real adoptable dogs scraped from
 * each partner's site (Bobbi, AMA, Animal Haven). Badass and Social Tees list
 * their dogs through external platforms (Petfinder / Adopt-a-Pet) so their
 * dogs are added manually from /admin → Shelters.
 *
 * Firestore ALWAYS wins: editing shelters in /admin (or running
 * scripts/import_shelters.mjs) overrides this default.
 */
